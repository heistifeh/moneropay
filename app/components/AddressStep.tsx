...
export default function AddressStep({ onNext }: { onNext: () => void }) {
  const { current, attachPayout, loading, error } = useQuoteStore();
  const [addr, setAddr] = useState('');

  async function handleAttach() {
    if (!current?.publicId) return;
    await attachPayout(addr);
    onNext();
  }

  return (
    <div>
      <div>Send funds to: <code>{current?.depositAddress ?? '—'}</code></div>
      <input value={addr} onChange={(e) => setAddr(e.target.value)} placeholder="Your payout address" />
      <button disabled={loading || !addr} onClick={handleAttach}>Continue</button>
