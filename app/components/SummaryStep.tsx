...
export default function SummaryStep() {
  const { current, subscribeToQuote, markUserPaid, loading, error } =
    useQuoteStore();
  const [tx, setTx] = useState("");

  useEffect(() => {
    if (!current?.publicId) return;
    const unsub = subscribeToQuote(current.publicId);
    return () => unsub();
  }, [current?.publicId, subscribeToQuote]);

  async function handlePaid() {
    if (!current?.publicId || !tx) return;
    await markUserPaid(tx);
  }
  ...
  return (
    <div>
      ...
      <div>
        Deposit: <code>{current?.depositAddress ?? "—"}</code>
      </div>
      <div>Expires: {current?.expiresAt ?? "—"}</div>
      ...
      />
      <button disabled={loading || expired || !tx} onClick={handlePaid}>
        I’ve sent the funds
      </button>

      {expired && <p style={{ color: "orange" }}>This quote has expired.</p>}
